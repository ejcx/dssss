...
### GET /v1/secret/{secretname}
Fetch a secret from the store. Requires an authentication or reauthentication header.
```
$ curl http://127.0.0.1:8000/v1/secret/password -H "Reauthentication: $(cat ~/.dssss-token)"
...
Put a secret in the store. Requires an authentication or reauthentication header.
```
$ curl http://127.0.0.1:8000/v1/secret/password -H "Reauthentication: $(cat ~/.dssss-token)" \
  && -XPUT -d "{\"Secret\":\"hunter2\", \"Roles\":[\"ecsInstanceRole\"]}"
```

### DELETE /v1/secret/{secretname}
Delete a secret in the store. Requires an authentication or reauthentication header *and a distinguished role*
```
$ curl http://127.0.0.1:8000/v1/secret/password -XDELETE -H "Reauthentication: $(cat ~/.dssss-token)"
```

### LIST /v1/secret/{secretname}
List secrets from the store. Requires an authentication or reauthentication header.
```
$ curl http://127.0.0.1:8000/v1/secret/pass -XLIST-H "Reauthentication: $(cat ~/.dssss-token)"
